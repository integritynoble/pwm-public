# Computed Tomography (CT) — Complete Four-Layer Walkthrough

**Principle #29: Computed Tomography (CT)**
Domain: Medical Imaging | Difficulty: Standard (delta=3) | Carrier: Photon
Verification: draft (identity-polished; physics body still widefield-templated) — X-ray photon transmission at many angles; Radon-transform forward model; FBP and iterative MBIR baselines. Phase F re-author queued.

---

## The Four-Layer Pipeline for Ct
...
```
┌────────────────────────────────────────────────────────────────────────────┐
│  CT PRINCIPLE  P = (E, G, W, C)                      │
│  Principle #29 in the PWM registry                                        │
│  sha256: <ct_principle_hash>  (immutable once committed)          │
├────────┬───────────────────────────────────────────────────────────────────┤
│   E    │ FORWARD MODEL                                                    │
│        │                                                                  │
...
```
Ct Principle (sha256:<principle_hash>)
    │
...
```yaml
# ct/fluocells_mismatch_only.yaml
# Layer 2 output — references the Ct Principle

principle_ref: sha256:<ct_principle_hash>   # ← links to Layer 1
...
```yaml
# ct/fluocells_oracle_assisted.yaml

principle_ref: sha256:<ct_principle_hash>
...
Each spec.md is validated against the Ct Principle:
...
```
benchmark_ct_mismatch_only_t1_nominal/   ← I-benchmark T1
...
```yaml
# benchmark_ct_mismatch_only_t1_nominal/manifest.yaml

benchmark_id:    "ct_mismatch_only_t1_nominal_v1"
type:            "I-benchmark"
spec_ref:        "sha256:<ct_spec1_hash>"       # mismatch-only spec
principle_ref:   "sha256:<ct_principle_hash>"
...
dataset_hash:    "sha256:<ct_dataset_hash>"
generator_hash:  "sha256:<ct_gen_hash>"
...
```bash
pwm-node verify ct/fluocells_t1_nominal.yaml
```
...
```bash
pwm-node mine ct/fluocells_t1_nominal.yaml
```
...
{
  "cert_hash": "sha256:...",
  "h_s": "sha256:<ct_spec1_hash>",
  "h_b": "sha256:<ct_bench1_hash>",
  "h_p": "sha256:<ct_principle_hash>",
...
```
cert references:
  h_p → Principle  sha256:<ct_principle_hash>   (Layer 1, immutable)
  h_s → spec.md    sha256:<ct_spec1_hash>       (Layer 2, immutable)
  h_b → Benchmark  sha256:<ct_bench1_hash>      (Layer 3, immutable)
  h_x → Solution   sha256:<ct_solution_hash>    (Layer 4, this submission)
```
...
```
Layer 1 ──→ Principle sha256:<ct_principle_hash>     FIXED
               │
Layer 2 ──→ spec.md sha256:<ct_spec1_hash>           FIXED
               │   contains: principle_ref: sha256:<ct_principle_hash>
               │
Layer 3 ──→ Benchmark sha256:<ct_bench1_hash>        FIXED
               │   contains: spec_ref: sha256:<ct_spec1_hash>
               │             principle_ref: sha256:<ct_principle_hash>
               │
Layer 4 ──→ Certificate sha256:<ct_cert_hash>        SUBMITTED
                   contains: h_s: sha256:<ct_spec1_hash>
                             h_b: sha256:<ct_bench1_hash>
                             h_p: sha256:<ct_principle_hash>
                             h_x: sha256:<ct_solution_hash>
...
```bash
pwm-node mine ct/fluocells_mismatch_only_t1_nominal.yaml
```
...
## Complete Reward Summary (All Four Layers for Ct)
...
pwm-node verify ct/mismatch_only_t1_nominal.yaml

# 3. Mine the center I-benchmark (nominal, ρ=1)
pwm-node mine ct/mismatch_only_t1_nominal.yaml
...
pwm-node mine ct/mismatch_only_t3_moderate.yaml

# 7. Mine blind calibration tier (ρ=10, highest I-benchmark)
pwm-node mine ct/mismatch_only_t4_blind.yaml

# 8. Mine oracle-assisted center I-benchmark (ρ=1, true_phi provided)
pwm-node mine ct/oracle_t1_h128_pix100.yaml
...
|-------|------------------|
| Ct Principle (#1) | `principle.md`, Section A (Microscopy) |
