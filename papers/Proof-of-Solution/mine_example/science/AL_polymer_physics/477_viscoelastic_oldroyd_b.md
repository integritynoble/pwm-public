# Viscoelastic Oldroyd B — Complete Four-Layer Walkthrough

**Principle #477: Viscoelastic Oldroyd B**
Domain: Microscopy | Difficulty: Standard (delta=2) | Carrier: Photon
Verification: draft — base-copy from pilot; needs physics + numerics + cross-domain verifier sign-off

---

## The Four-Layer Pipeline for Viscoelastic Oldroyd B
...
```
┌────────────────────────────────────────────────────────────────────────────┐
│  VISCOELASTIC OLDROYD B PRINCIPLE  P = (E, G, W, C)                      │
│  Principle #477 in the PWM registry                                        │
│  sha256: <viscoelastic_oldroyd_b_principle_hash>  (immutable once committed)          │
├────────┬───────────────────────────────────────────────────────────────────┤
│   E    │ FORWARD MODEL                                                    │
│        │                                                                  │
...
```
Viscoelastic Oldroyd B Principle (sha256:<principle_hash>)
    │
...
```yaml
# viscoelastic_oldroyd_b/fluocells_mismatch_only.yaml
# Layer 2 output — references the Viscoelastic Oldroyd B Principle

principle_ref: sha256:<viscoelastic_oldroyd_b_principle_hash>   # ← links to Layer 1
...
```yaml
# viscoelastic_oldroyd_b/fluocells_oracle_assisted.yaml

principle_ref: sha256:<viscoelastic_oldroyd_b_principle_hash>
...
Each spec.md is validated against the Viscoelastic Oldroyd B Principle:
...
```
benchmark_viscoelastic_oldroyd_b_mismatch_only_t1_nominal/   ← I-benchmark T1
...
```yaml
# benchmark_viscoelastic_oldroyd_b_mismatch_only_t1_nominal/manifest.yaml

benchmark_id:    "viscoelastic_oldroyd_b_mismatch_only_t1_nominal_v1"
type:            "I-benchmark"
spec_ref:        "sha256:<viscoelastic_oldroyd_b_spec1_hash>"       # mismatch-only spec
principle_ref:   "sha256:<viscoelastic_oldroyd_b_principle_hash>"
...
dataset_hash:    "sha256:<viscoelastic_oldroyd_b_dataset_hash>"
generator_hash:  "sha256:<viscoelastic_oldroyd_b_gen_hash>"
...
```bash
pwm-node benchmarks | grep viscoelastic_oldroyd_b
```
...
```bash
pwm-node verify viscoelastic_oldroyd_b/fluocells_t1_nominal.yaml
```
...
```bash
pwm-node mine viscoelastic_oldroyd_b/fluocells_t1_nominal.yaml
```
...
{
  "cert_hash": "sha256:...",
  "h_s": "sha256:<viscoelastic_oldroyd_b_spec1_hash>",
  "h_b": "sha256:<viscoelastic_oldroyd_b_bench1_hash>",
  "h_p": "sha256:<viscoelastic_oldroyd_b_principle_hash>",
...
```
cert references:
  h_p → Principle  sha256:<viscoelastic_oldroyd_b_principle_hash>   (Layer 1, immutable)
  h_s → spec.md    sha256:<viscoelastic_oldroyd_b_spec1_hash>       (Layer 2, immutable)
  h_b → Benchmark  sha256:<viscoelastic_oldroyd_b_bench1_hash>      (Layer 3, immutable)
  h_x → Solution   sha256:<viscoelastic_oldroyd_b_solution_hash>    (Layer 4, this submission)
```
...
```
Layer 1 ──→ Principle sha256:<viscoelastic_oldroyd_b_principle_hash>     FIXED
               │
Layer 2 ──→ spec.md sha256:<viscoelastic_oldroyd_b_spec1_hash>           FIXED
               │   contains: principle_ref: sha256:<viscoelastic_oldroyd_b_principle_hash>
               │
Layer 3 ──→ Benchmark sha256:<viscoelastic_oldroyd_b_bench1_hash>        FIXED
               │   contains: spec_ref: sha256:<viscoelastic_oldroyd_b_spec1_hash>
               │             principle_ref: sha256:<viscoelastic_oldroyd_b_principle_hash>
               │
Layer 4 ──→ Certificate sha256:<viscoelastic_oldroyd_b_cert_hash>        SUBMITTED
                   contains: h_s: sha256:<viscoelastic_oldroyd_b_spec1_hash>
                             h_b: sha256:<viscoelastic_oldroyd_b_bench1_hash>
                             h_p: sha256:<viscoelastic_oldroyd_b_principle_hash>
                             h_x: sha256:<viscoelastic_oldroyd_b_solution_hash>
...
```bash
pwm-node mine viscoelastic_oldroyd_b/fluocells_mismatch_only_t1_nominal.yaml
```
...
## Complete Reward Summary (All Four Layers for Viscoelastic Oldroyd B)
...
pwm-node verify viscoelastic_oldroyd_b/mismatch_only_t1_nominal.yaml

# 3. Mine the center I-benchmark (nominal, ρ=1)
pwm-node mine viscoelastic_oldroyd_b/mismatch_only_t1_nominal.yaml
...
pwm-node mine viscoelastic_oldroyd_b/mismatch_only_t3_moderate.yaml

# 7. Mine blind calibration tier (ρ=10, highest I-benchmark)
pwm-node mine viscoelastic_oldroyd_b/mismatch_only_t4_blind.yaml

# 8. Mine oracle-assisted center I-benchmark (ρ=1, true_phi provided)
pwm-node mine viscoelastic_oldroyd_b/oracle_t1_h128_pix100.yaml
...
|-------|------------------|
| Viscoelastic Oldroyd B Principle (#1) | `principle.md`, Section A (Microscopy) |
