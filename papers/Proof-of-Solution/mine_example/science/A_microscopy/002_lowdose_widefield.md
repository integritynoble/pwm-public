# Low-Dose Low-Dose Widefield Fluorescence Microscopy — Complete Four-Layer Walkthrough

**Principle #2: Low-Dose Low-Dose Widefield Fluorescence Microscopy**
Domain: Microscopy | Difficulty: Standard (delta=3) | Carrier: Photon

---

## The Four-Layer Pipeline for Low-Dose Widefield Fluorescence
...
```
┌────────────────────────────────────────────────────────────────────────────┐
│  LOW-DOSE WIDEFIELD FLUORESCENCE PRINCIPLE  P = (E, G, W, C)                      │
│  Principle #2 in the PWM registry                                        │
│  sha256: <lowdose_widefield_principle_hash>  (immutable once committed)          │
├────────┬───────────────────────────────────────────────────────────────────┤
│   E    │ FORWARD MODEL                                                    │
│        │                                                                  │
...
│        │   K.psf.airy: kernel structure, κ ≈ 50 (SNR-inflated at low     │
│        │               photon counts; same Airy OTF as standard           │
│        │               widefield but noise amplifies conditioning cost)   │
│        │   ∫.temporal: low-count Poisson noise accumulator, κ ≈ 1,        │
│        │               shot noise is the dominant output-side constraint │
...
│        │   Low-dose regime reveals the *noise-amplification*              │
│        │   characteristic of κ-ill-conditioned inversions — same pattern │
│        │   shows up in cryo-EM, low-dose CT, and photon-starved lidar.   │
│        │                                                                  │
│        │ L_DAG = (|V|-1) + max(0, log₁₀(κ_sys/κ₀)) + n_c                 │
│        │       = 1 + max(0, log₁₀(120/10)) + 0                            │
│        │       = 1 + 1.08 + 0 ≈ 2.1   (κ₀ = 10 for photon-limited         │
│        │                                regimes; κ_sys ≈ 120 reflects     │
│        │                                shot-noise amplification, so the │
│        │                                log term no longer floors at 0)   │
│        │                                                                  │
...
│        │                                                                  │
│        │ Stability: CONDITIONAL — SHOT-NOISE-LIMITED regime               │
│        │   Sub-operator κ_∫ ≈ 1  (∫.temporal adds Poisson noise, not      │
│        │                          conditioning cost)                      │
│        │   Sub-operator κ_K ≈ 50 (K.psf.airy, SNR-inflated at peak≈50     │
│        │                          photons; κ balloons as SNR drops)       │
│        │   Compound κ_sys ≈ 120  (shot-noise regime, 50 photon/pixel)    │
│        │   Effective κ_eff ≈ 30  (learned prior — N2V, HDN, CARE — at     │
│        │                          typical low-dose SNR)                   │
│        │                                                                  │
│        │ Mismatch model (3 parameters as Ω dimensions):                   │
│        │   y_observed = Poisson(K_{Δz} · f) + σ_bg + N(0, σ_read²)        │
│        │   where Δz      = axial defocus (introduces OTF phase error)     │
│        │         σ_bg    = uniform background offset (fraction of peak)   │
│        │         σ_read  = read-noise std (e⁻ per pixel; camera-dep.)     │
│        │   Nominal: Δz = 0, σ_bg = 0, σ_read = 1.5 e⁻                    │
│        │   Mismatch bounds: Δz ∈ [0, 1500] nm, σ_bg ∈ [0, 0.10],          │
│        │                    σ_read ∈ [0.5, 5.0] e⁻                        │
│        │                                                                  │
│        │ Note: low-dose widefield adds σ_read vs. standard widefield's 2  │
│        │ mismatch dims. Read noise is irrelevant at high peak_photons     │
│        │ but becomes a dominant nuisance parameter when peak ≤ 100.       │
├────────┼───────────────────────────────────────────────────────────────────┤
...
```
Low-Dose Widefield Fluorescence Principle (sha256:<principle_hash>)
    │
...
```yaml
# lowdose_widefield/fluocells_mismatch_only.yaml
# Layer 2 output — references the Low-Dose Widefield Fluorescence Principle

principle_ref: sha256:<lowdose_widefield_principle_hash>   # ← links to Layer 1
...
```yaml
# lowdose_widefield/fluocells_oracle_assisted.yaml

principle_ref: sha256:<lowdose_widefield_principle_hash>
...
Each spec.md is validated against the Low-Dose Widefield Fluorescence Principle:
...
```
benchmark_lowdose_widefield_mismatch_only_t1_nominal/   ← I-benchmark T1
...
```yaml
# benchmark_lowdose_widefield_mismatch_only_t1_nominal/manifest.yaml

benchmark_id:    "lowdose_widefield_mismatch_only_t1_nominal_v1"
type:            "I-benchmark"
spec_ref:        "sha256:<lowdose_widefield_spec1_hash>"       # mismatch-only spec
principle_ref:   "sha256:<lowdose_widefield_principle_hash>"
...
dataset_hash:    "sha256:<lowdose_widefield_dataset_hash>"
generator_hash:  "sha256:<lowdose_widefield_gen_hash>"
...
```bash
 grep widefield| grep widefield
 grep lowdose_widefield| grep lowdose_widefield
```
...
```bash
pwm-node verify lowdose_widefield/fluocells_t1_nominal.yaml
```
...
```bash
pwm-node mine lowdose_widefield/fluocells_t1_nominal.yaml
```
...
| CARE-UNet         | ~36 dB | 0.5 s/scene  | 0.98 | Learned, pretrained on BioSR_LowDose |
...
{
  "cert_hash": "sha256:...",
  "h_s": "sha256:<lowdose_widefield_spec1_hash>",
  "h_b": "sha256:<lowdose_widefield_bench1_hash>",
  "h_p": "sha256:<lowdose_widefield_principle_hash>",
...
```
cert references:
  h_p → Principle  sha256:<lowdose_widefield_principle_hash>   (Layer 1, immutable)
  h_s → spec.md    sha256:<lowdose_widefield_spec1_hash>       (Layer 2, immutable)
  h_b → Benchmark  sha256:<lowdose_widefield_bench1_hash>      (Layer 3, immutable)
  h_x → Solution   sha256:<lowdose_widefield_solution_hash>    (Layer 4, this submission)
```
...
```
Layer 1 ──→ Principle sha256:<lowdose_widefield_principle_hash>     FIXED
               │
Layer 2 ──→ spec.md sha256:<lowdose_widefield_spec1_hash>           FIXED
               │   contains: principle_ref: sha256:<lowdose_widefield_principle_hash>
               │
Layer 3 ──→ Benchmark sha256:<lowdose_widefield_bench1_hash>        FIXED
               │   contains: spec_ref: sha256:<lowdose_widefield_spec1_hash>
               │             principle_ref: sha256:<lowdose_widefield_principle_hash>
               │
Layer 4 ──→ Certificate sha256:<lowdose_widefield_cert_hash>        SUBMITTED
                   contains: h_s: sha256:<lowdose_widefield_spec1_hash>
                             h_b: sha256:<lowdose_widefield_bench1_hash>
                             h_p: sha256:<lowdose_widefield_principle_hash>
                             h_x: sha256:<lowdose_widefield_solution_hash>
...
```bash
pwm-node mine lowdose_widefield/fluocells_mismatch_only_t1_nominal.yaml
```
...
| ε | 22.0 dB |
| ρ | 10 |
| Dataset | BioSR_LowDose (40 scenes, 1024×1024 intensity + ground-truth fluorophore maps); cropped and resampled to 512×512; synthetic defocus + background applied at T4 parameters; fixed 20-scene eval set |
...
**Source dataset:** BioSR_LowDose-XL (500 scenes, 2048×2048 intensity + ground-truth fluorophore maps, CC-BY-NC). A subset of the Broad Bioimage Benchmark Collection's synthetic fluorescence library augmented with real confocal-derived ground truths for pairwise comparison.

**Stitching for H×W > 1024:** Real widefield deployments tile multiple fields of view for large-area imaging (whole-well scans, tissue slides). The P-benchmark mirrors this: for target H or W > 1024, four BioSR_LowDose-XL scenes are tiled in a 2×2 arrangement (hard concatenation, no blending) and cropped to the target size. The seam at tile boundaries is a real spatial discontinuity — solvers that handle it score higher. BioSR_LowDose-XL at 2048×2048 per tile covers 2×2 → 4096×4096, sufficient for any target up to 4096.

| Target H×W | Tile count | Source |
|---|---|---|
| [128, 1024] | 1 BioSR_LowDose-XL crop | No stitching |
| (1024, 4096] | 4 BioSR_LowDose-XL scenes (2×2 hard stitch) | Seam at tile boundary |
...
## Complete Reward Summary (All Four Layers for Low-Dose Widefield Fluorescence)
...
pwm-node verify lowdose_widefield/mismatch_only_t1_nominal.yaml

# 3. Mine the center I-benchmark (nominal, ρ=1)
pwm-node mine lowdose_widefield/mismatch_only_t1_nominal.yaml
...
pwm-node mine lowdose_widefield/mismatch_only_t3_moderate.yaml

# 7. Mine blind calibration tier (ρ=10, highest I-benchmark)
pwm-node mine lowdose_widefield/mismatch_only_t4_blind.yaml

# 8. Mine oracle-assisted center I-benchmark (ρ=1, true_phi provided)
pwm-node mine lowdose_widefield/oracle_t1_h128_pix100.yaml
...
|-------|------------------|
| Low-Dose Widefield Fluorescence Principle (#1) | `principle.md`, Section A (Microscopy) |
...
| BioSR_LowDose dataset | Broad Bioimage Benchmark Collection ⟨draft — pending exact DOI / collection ID⟩ |
