# Fourier Ptychographic Microscopy (FPM) — Complete Four-Layer Walkthrough
...
## The Four-Layer Pipeline for Fourier Ptychographic Microscopy (FPM)
...
```
┌────────────────────────────────────────────────────────────────────────────┐
│  FOURIER PTYCHOGRAPHIC MICROSCOPY (FPM) PRINCIPLE  P = (E, G, W, C)                      │
│  Principle #8 in the PWM registry                                        │
│  sha256: <fpm_principle_hash>  (immutable once committed)          │
├────────┬───────────────────────────────────────────────────────────────────┤
│   E    │ FORWARD MODEL                                                    │
│        │                                                                  │
...
```
Fourier Ptychographic Microscopy (FPM) Principle (sha256:<principle_hash>)
    │
...
```yaml
# fpm/fluocells_mismatch_only.yaml
# Layer 2 output — references the Fourier Ptychographic Microscopy (FPM) Principle

principle_ref: sha256:<fpm_principle_hash>   # ← links to Layer 1
...
```yaml
# fpm/fluocells_oracle_assisted.yaml

principle_ref: sha256:<fpm_principle_hash>
...
Each spec.md is validated against the Fourier Ptychographic Microscopy (FPM) Principle:
...
```
benchmark_fpm_mismatch_only_t1_nominal/   ← I-benchmark T1
...
```yaml
# benchmark_fpm_mismatch_only_t1_nominal/manifest.yaml

benchmark_id:    "fpm_mismatch_only_t1_nominal_v1"
type:            "I-benchmark"
spec_ref:        "sha256:<fpm_spec1_hash>"       # mismatch-only spec
principle_ref:   "sha256:<fpm_principle_hash>"
...
dataset_hash:    "sha256:<fpm_dataset_hash>"
generator_hash:  "sha256:<fpm_gen_hash>"
...
```bash
pwm-node verify fpm/fluocells_t1_nominal.yaml
```
...
```bash
pwm-node mine fpm/fluocells_t1_nominal.yaml
```
...
{
  "cert_hash": "sha256:...",
  "h_s": "sha256:<fpm_spec1_hash>",
  "h_b": "sha256:<fpm_bench1_hash>",
  "h_p": "sha256:<fpm_principle_hash>",
...
```
cert references:
  h_p → Principle  sha256:<fpm_principle_hash>   (Layer 1, immutable)
  h_s → spec.md    sha256:<fpm_spec1_hash>       (Layer 2, immutable)
  h_b → Benchmark  sha256:<fpm_bench1_hash>      (Layer 3, immutable)
  h_x → Solution   sha256:<fpm_solution_hash>    (Layer 4, this submission)
```
...
```
Layer 1 ──→ Principle sha256:<fpm_principle_hash>     FIXED
               │
Layer 2 ──→ spec.md sha256:<fpm_spec1_hash>           FIXED
               │   contains: principle_ref: sha256:<fpm_principle_hash>
               │
Layer 3 ──→ Benchmark sha256:<fpm_bench1_hash>        FIXED
               │   contains: spec_ref: sha256:<fpm_spec1_hash>
               │             principle_ref: sha256:<fpm_principle_hash>
               │
Layer 4 ──→ Certificate sha256:<fpm_cert_hash>        SUBMITTED
                   contains: h_s: sha256:<fpm_spec1_hash>
                             h_b: sha256:<fpm_bench1_hash>
                             h_p: sha256:<fpm_principle_hash>
                             h_x: sha256:<fpm_solution_hash>
...
```bash
pwm-node mine fpm/fluocells_mismatch_only_t1_nominal.yaml
```
...
## Complete Reward Summary (All Four Layers for Fourier Ptychographic Microscopy (FPM))
...
pwm-node verify fpm/mismatch_only_t1_nominal.yaml

# 3. Mine the center I-benchmark (nominal, ρ=1)
pwm-node mine fpm/mismatch_only_t1_nominal.yaml
...
pwm-node mine fpm/mismatch_only_t3_moderate.yaml

# 7. Mine blind calibration tier (ρ=10, highest I-benchmark)
pwm-node mine fpm/mismatch_only_t4_blind.yaml

# 8. Mine oracle-assisted center I-benchmark (ρ=1, true_phi provided)
pwm-node mine fpm/oracle_t1_h128_pix100.yaml
...
|-------|------------------|
| Fourier Ptychographic Microscopy (FPM) Principle (#1) | `principle.md`, Section A (Microscopy) |
