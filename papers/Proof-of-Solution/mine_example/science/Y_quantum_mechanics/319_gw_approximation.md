# Gw Approximation — Complete Four-Layer Walkthrough

**Principle #319: Gw Approximation**
Domain: Microscopy | Difficulty: Standard (delta=2) | Carrier: Photon
Verification: draft — base-copy from pilot; needs physics + numerics + cross-domain verifier sign-off

---

## The Four-Layer Pipeline for Gw Approximation
...
```
┌────────────────────────────────────────────────────────────────────────────┐
│  GW APPROXIMATION PRINCIPLE  P = (E, G, W, C)                      │
│  Principle #319 in the PWM registry                                        │
│  sha256: <gw_approximation_principle_hash>  (immutable once committed)          │
├────────┬───────────────────────────────────────────────────────────────────┤
│   E    │ FORWARD MODEL                                                    │
│        │                                                                  │
...
```
Gw Approximation Principle (sha256:<principle_hash>)
    │
...
```yaml
# gw_approximation/fluocells_mismatch_only.yaml
# Layer 2 output — references the Gw Approximation Principle

principle_ref: sha256:<gw_approximation_principle_hash>   # ← links to Layer 1
...
```yaml
# gw_approximation/fluocells_oracle_assisted.yaml

principle_ref: sha256:<gw_approximation_principle_hash>
...
Each spec.md is validated against the Gw Approximation Principle:
...
```
benchmark_gw_approximation_mismatch_only_t1_nominal/   ← I-benchmark T1
...
```yaml
# benchmark_gw_approximation_mismatch_only_t1_nominal/manifest.yaml

benchmark_id:    "gw_approximation_mismatch_only_t1_nominal_v1"
type:            "I-benchmark"
spec_ref:        "sha256:<gw_approximation_spec1_hash>"       # mismatch-only spec
principle_ref:   "sha256:<gw_approximation_principle_hash>"
...
dataset_hash:    "sha256:<gw_approximation_dataset_hash>"
generator_hash:  "sha256:<gw_approximation_gen_hash>"
...
```bash
pwm-node benchmarks | grep gw_approximation
```
...
```bash
pwm-node verify gw_approximation/fluocells_t1_nominal.yaml
```
...
```bash
pwm-node mine gw_approximation/fluocells_t1_nominal.yaml
```
...
{
  "cert_hash": "sha256:...",
  "h_s": "sha256:<gw_approximation_spec1_hash>",
  "h_b": "sha256:<gw_approximation_bench1_hash>",
  "h_p": "sha256:<gw_approximation_principle_hash>",
...
```
cert references:
  h_p → Principle  sha256:<gw_approximation_principle_hash>   (Layer 1, immutable)
  h_s → spec.md    sha256:<gw_approximation_spec1_hash>       (Layer 2, immutable)
  h_b → Benchmark  sha256:<gw_approximation_bench1_hash>      (Layer 3, immutable)
  h_x → Solution   sha256:<gw_approximation_solution_hash>    (Layer 4, this submission)
```
...
```
Layer 1 ──→ Principle sha256:<gw_approximation_principle_hash>     FIXED
               │
Layer 2 ──→ spec.md sha256:<gw_approximation_spec1_hash>           FIXED
               │   contains: principle_ref: sha256:<gw_approximation_principle_hash>
               │
Layer 3 ──→ Benchmark sha256:<gw_approximation_bench1_hash>        FIXED
               │   contains: spec_ref: sha256:<gw_approximation_spec1_hash>
               │             principle_ref: sha256:<gw_approximation_principle_hash>
               │
Layer 4 ──→ Certificate sha256:<gw_approximation_cert_hash>        SUBMITTED
                   contains: h_s: sha256:<gw_approximation_spec1_hash>
                             h_b: sha256:<gw_approximation_bench1_hash>
                             h_p: sha256:<gw_approximation_principle_hash>
                             h_x: sha256:<gw_approximation_solution_hash>
...
```bash
pwm-node mine gw_approximation/fluocells_mismatch_only_t1_nominal.yaml
```
...
## Complete Reward Summary (All Four Layers for Gw Approximation)
...
pwm-node verify gw_approximation/mismatch_only_t1_nominal.yaml

# 3. Mine the center I-benchmark (nominal, ρ=1)
pwm-node mine gw_approximation/mismatch_only_t1_nominal.yaml
...
pwm-node mine gw_approximation/mismatch_only_t3_moderate.yaml

# 7. Mine blind calibration tier (ρ=10, highest I-benchmark)
pwm-node mine gw_approximation/mismatch_only_t4_blind.yaml

# 8. Mine oracle-assisted center I-benchmark (ρ=1, true_phi provided)
pwm-node mine gw_approximation/oracle_t1_h128_pix100.yaml
...
|-------|------------------|
| Gw Approximation Principle (#1) | `principle.md`, Section A (Microscopy) |
