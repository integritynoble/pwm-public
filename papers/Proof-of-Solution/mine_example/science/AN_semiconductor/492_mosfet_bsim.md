# Mosfet Bsim — Complete Four-Layer Walkthrough

**Principle #492: Mosfet Bsim**
Domain: Microscopy | Difficulty: Standard (delta=2) | Carrier: Photon

---

## The Four-Layer Pipeline for Mosfet Bsim
...
```
┌────────────────────────────────────────────────────────────────────────────┐
│  MOSFET BSIM PRINCIPLE  P = (E, G, W, C)                      │
│  Principle #492 in the PWM registry                                        │
│  sha256: <mosfet_bsim_principle_hash>  (immutable once committed)          │
├────────┬───────────────────────────────────────────────────────────────────┤
│   E    │ FORWARD MODEL                                                    │
│        │                                                                  │
...
```
Mosfet Bsim Principle (sha256:<principle_hash>)
    │
...
```yaml
# mosfet_bsim/fluocells_mismatch_only.yaml
# Layer 2 output — references the Mosfet Bsim Principle

principle_ref: sha256:<mosfet_bsim_principle_hash>   # ← links to Layer 1
...
```yaml
# mosfet_bsim/fluocells_oracle_assisted.yaml

principle_ref: sha256:<mosfet_bsim_principle_hash>
...
Each spec.md is validated against the Mosfet Bsim Principle:
...
```
benchmark_mosfet_bsim_mismatch_only_t1_nominal/   ← I-benchmark T1
...
```yaml
# benchmark_mosfet_bsim_mismatch_only_t1_nominal/manifest.yaml

benchmark_id:    "mosfet_bsim_mismatch_only_t1_nominal_v1"
type:            "I-benchmark"
spec_ref:        "sha256:<mosfet_bsim_spec1_hash>"       # mismatch-only spec
principle_ref:   "sha256:<mosfet_bsim_principle_hash>"
...
dataset_hash:    "sha256:<mosfet_bsim_dataset_hash>"
generator_hash:  "sha256:<mosfet_bsim_gen_hash>"
...
```bash
pwm-node benchmarks | grep mosfet_bsim
```
...
```bash
pwm-node verify mosfet_bsim/fluocells_t1_nominal.yaml
```
...
```bash
pwm-node mine mosfet_bsim/fluocells_t1_nominal.yaml
```
...
{
  "cert_hash": "sha256:...",
  "h_s": "sha256:<mosfet_bsim_spec1_hash>",
  "h_b": "sha256:<mosfet_bsim_bench1_hash>",
  "h_p": "sha256:<mosfet_bsim_principle_hash>",
...
```
cert references:
  h_p → Principle  sha256:<mosfet_bsim_principle_hash>   (Layer 1, immutable)
  h_s → spec.md    sha256:<mosfet_bsim_spec1_hash>       (Layer 2, immutable)
  h_b → Benchmark  sha256:<mosfet_bsim_bench1_hash>      (Layer 3, immutable)
  h_x → Solution   sha256:<mosfet_bsim_solution_hash>    (Layer 4, this submission)
```
...
```
Layer 1 ──→ Principle sha256:<mosfet_bsim_principle_hash>     FIXED
               │
Layer 2 ──→ spec.md sha256:<mosfet_bsim_spec1_hash>           FIXED
               │   contains: principle_ref: sha256:<mosfet_bsim_principle_hash>
               │
Layer 3 ──→ Benchmark sha256:<mosfet_bsim_bench1_hash>        FIXED
               │   contains: spec_ref: sha256:<mosfet_bsim_spec1_hash>
               │             principle_ref: sha256:<mosfet_bsim_principle_hash>
               │
Layer 4 ──→ Certificate sha256:<mosfet_bsim_cert_hash>        SUBMITTED
                   contains: h_s: sha256:<mosfet_bsim_spec1_hash>
                             h_b: sha256:<mosfet_bsim_bench1_hash>
                             h_p: sha256:<mosfet_bsim_principle_hash>
                             h_x: sha256:<mosfet_bsim_solution_hash>
...
```bash
pwm-node mine mosfet_bsim/fluocells_mismatch_only_t1_nominal.yaml
```
...
## Complete Reward Summary (All Four Layers for Mosfet Bsim)
...
pwm-node verify mosfet_bsim/mismatch_only_t1_nominal.yaml

# 3. Mine the center I-benchmark (nominal, ρ=1)
pwm-node mine mosfet_bsim/mismatch_only_t1_nominal.yaml
...
pwm-node mine mosfet_bsim/mismatch_only_t3_moderate.yaml

# 7. Mine blind calibration tier (ρ=10, highest I-benchmark)
pwm-node mine mosfet_bsim/mismatch_only_t4_blind.yaml

# 8. Mine oracle-assisted center I-benchmark (ρ=1, true_phi provided)
pwm-node mine mosfet_bsim/oracle_t1_h128_pix100.yaml
...
|-------|------------------|
| Mosfet Bsim Principle (#1) | `principle.md`, Section A (Microscopy) |
