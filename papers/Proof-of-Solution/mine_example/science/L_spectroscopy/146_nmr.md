# Nuclear Magnetic Resonance (NMR) Spectroscopy — Complete Four-Layer Walkthrough

**Principle #146: Nuclear Magnetic Resonance (NMR) Spectroscopy**
Domain: Spectroscopy | Difficulty: Advanced (delta=4) | Carrier: Field
Verification: draft (identity-polished; physics body still widefield-templated) — Zeeman-split nuclear spin transitions excited by RF pulses; chemical shift encodes electronic environment; MRI's analytical sibling. Phase F re-author queued.

---

## The Four-Layer Pipeline for Nmr
...
```
┌────────────────────────────────────────────────────────────────────────────┐
│  NMR PRINCIPLE  P = (E, G, W, C)                      │
│  Principle #146 in the PWM registry                                        │
│  sha256: <nmr_principle_hash>  (immutable once committed)          │
├────────┬───────────────────────────────────────────────────────────────────┤
│   E    │ FORWARD MODEL                                                    │
│        │                                                                  │
...
```
Nmr Principle (sha256:<principle_hash>)
    │
...
```yaml
# nmr/fluocells_mismatch_only.yaml
# Layer 2 output — references the Nmr Principle

principle_ref: sha256:<nmr_principle_hash>   # ← links to Layer 1
...
```yaml
# nmr/fluocells_oracle_assisted.yaml

principle_ref: sha256:<nmr_principle_hash>
...
Each spec.md is validated against the Nmr Principle:
...
```
benchmark_nmr_mismatch_only_t1_nominal/   ← I-benchmark T1
...
```yaml
# benchmark_nmr_mismatch_only_t1_nominal/manifest.yaml

benchmark_id:    "nmr_mismatch_only_t1_nominal_v1"
type:            "I-benchmark"
spec_ref:        "sha256:<nmr_spec1_hash>"       # mismatch-only spec
principle_ref:   "sha256:<nmr_principle_hash>"
...
dataset_hash:    "sha256:<nmr_dataset_hash>"
generator_hash:  "sha256:<nmr_gen_hash>"
...
```bash
pwm-node verify nmr/fluocells_t1_nominal.yaml
```
...
```bash
pwm-node mine nmr/fluocells_t1_nominal.yaml
```
...
{
  "cert_hash": "sha256:...",
  "h_s": "sha256:<nmr_spec1_hash>",
  "h_b": "sha256:<nmr_bench1_hash>",
  "h_p": "sha256:<nmr_principle_hash>",
...
```
cert references:
  h_p → Principle  sha256:<nmr_principle_hash>   (Layer 1, immutable)
  h_s → spec.md    sha256:<nmr_spec1_hash>       (Layer 2, immutable)
  h_b → Benchmark  sha256:<nmr_bench1_hash>      (Layer 3, immutable)
  h_x → Solution   sha256:<nmr_solution_hash>    (Layer 4, this submission)
```
...
```
Layer 1 ──→ Principle sha256:<nmr_principle_hash>     FIXED
               │
Layer 2 ──→ spec.md sha256:<nmr_spec1_hash>           FIXED
               │   contains: principle_ref: sha256:<nmr_principle_hash>
               │
Layer 3 ──→ Benchmark sha256:<nmr_bench1_hash>        FIXED
               │   contains: spec_ref: sha256:<nmr_spec1_hash>
               │             principle_ref: sha256:<nmr_principle_hash>
               │
Layer 4 ──→ Certificate sha256:<nmr_cert_hash>        SUBMITTED
                   contains: h_s: sha256:<nmr_spec1_hash>
                             h_b: sha256:<nmr_bench1_hash>
                             h_p: sha256:<nmr_principle_hash>
                             h_x: sha256:<nmr_solution_hash>
...
```bash
pwm-node mine nmr/fluocells_mismatch_only_t1_nominal.yaml
```
...
## Complete Reward Summary (All Four Layers for Nmr)
...
pwm-node verify nmr/mismatch_only_t1_nominal.yaml

# 3. Mine the center I-benchmark (nominal, ρ=1)
pwm-node mine nmr/mismatch_only_t1_nominal.yaml
...
pwm-node mine nmr/mismatch_only_t3_moderate.yaml

# 7. Mine blind calibration tier (ρ=10, highest I-benchmark)
pwm-node mine nmr/mismatch_only_t4_blind.yaml

# 8. Mine oracle-assisted center I-benchmark (ρ=1, true_phi provided)
pwm-node mine nmr/oracle_t1_h128_pix100.yaml
...
|-------|------------------|
| Nmr Principle (#1) | `principle.md`, Section A (Microscopy) |
