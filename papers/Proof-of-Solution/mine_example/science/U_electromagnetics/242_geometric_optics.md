# Geometric Optics — Complete Four-Layer Walkthrough

**Principle #242: Geometric Optics**
Domain: Microscopy | Difficulty: Standard (delta=2) | Carrier: Photon
Verification: draft — base-copy from pilot; needs physics + numerics + cross-domain verifier sign-off

---

## The Four-Layer Pipeline for Geometric Optics
...
```
┌────────────────────────────────────────────────────────────────────────────┐
│  GEOMETRIC OPTICS PRINCIPLE  P = (E, G, W, C)                      │
│  Principle #242 in the PWM registry                                        │
│  sha256: <geometric_optics_principle_hash>  (immutable once committed)          │
├────────┬───────────────────────────────────────────────────────────────────┤
│   E    │ FORWARD MODEL                                                    │
│        │                                                                  │
...
```
Geometric Optics Principle (sha256:<principle_hash>)
    │
...
```yaml
# geometric_optics/fluocells_mismatch_only.yaml
# Layer 2 output — references the Geometric Optics Principle

principle_ref: sha256:<geometric_optics_principle_hash>   # ← links to Layer 1
...
```yaml
# geometric_optics/fluocells_oracle_assisted.yaml

principle_ref: sha256:<geometric_optics_principle_hash>
...
Each spec.md is validated against the Geometric Optics Principle:
...
```
benchmark_geometric_optics_mismatch_only_t1_nominal/   ← I-benchmark T1
...
```yaml
# benchmark_geometric_optics_mismatch_only_t1_nominal/manifest.yaml

benchmark_id:    "geometric_optics_mismatch_only_t1_nominal_v1"
type:            "I-benchmark"
spec_ref:        "sha256:<geometric_optics_spec1_hash>"       # mismatch-only spec
principle_ref:   "sha256:<geometric_optics_principle_hash>"
...
dataset_hash:    "sha256:<geometric_optics_dataset_hash>"
generator_hash:  "sha256:<geometric_optics_gen_hash>"
...
```bash
pwm-node verify geometric_optics/fluocells_t1_nominal.yaml
```
...
```bash
pwm-node mine geometric_optics/fluocells_t1_nominal.yaml
```
...
{
  "cert_hash": "sha256:...",
  "h_s": "sha256:<geometric_optics_spec1_hash>",
  "h_b": "sha256:<geometric_optics_bench1_hash>",
  "h_p": "sha256:<geometric_optics_principle_hash>",
...
```
cert references:
  h_p → Principle  sha256:<geometric_optics_principle_hash>   (Layer 1, immutable)
  h_s → spec.md    sha256:<geometric_optics_spec1_hash>       (Layer 2, immutable)
  h_b → Benchmark  sha256:<geometric_optics_bench1_hash>      (Layer 3, immutable)
  h_x → Solution   sha256:<geometric_optics_solution_hash>    (Layer 4, this submission)
```
...
```
Layer 1 ──→ Principle sha256:<geometric_optics_principle_hash>     FIXED
               │
Layer 2 ──→ spec.md sha256:<geometric_optics_spec1_hash>           FIXED
               │   contains: principle_ref: sha256:<geometric_optics_principle_hash>
               │
Layer 3 ──→ Benchmark sha256:<geometric_optics_bench1_hash>        FIXED
               │   contains: spec_ref: sha256:<geometric_optics_spec1_hash>
               │             principle_ref: sha256:<geometric_optics_principle_hash>
               │
Layer 4 ──→ Certificate sha256:<geometric_optics_cert_hash>        SUBMITTED
                   contains: h_s: sha256:<geometric_optics_spec1_hash>
                             h_b: sha256:<geometric_optics_bench1_hash>
                             h_p: sha256:<geometric_optics_principle_hash>
                             h_x: sha256:<geometric_optics_solution_hash>
...
```bash
pwm-node mine geometric_optics/fluocells_mismatch_only_t1_nominal.yaml
```
...
## Complete Reward Summary (All Four Layers for Geometric Optics)
...
pwm-node verify geometric_optics/mismatch_only_t1_nominal.yaml

# 3. Mine the center I-benchmark (nominal, ρ=1)
pwm-node mine geometric_optics/mismatch_only_t1_nominal.yaml
...
pwm-node mine geometric_optics/mismatch_only_t3_moderate.yaml

# 7. Mine blind calibration tier (ρ=10, highest I-benchmark)
pwm-node mine geometric_optics/mismatch_only_t4_blind.yaml

# 8. Mine oracle-assisted center I-benchmark (ρ=1, true_phi provided)
pwm-node mine geometric_optics/oracle_t1_h128_pix100.yaml
...
|-------|------------------|
| Geometric Optics Principle (#1) | `principle.md`, Section A (Microscopy) |
