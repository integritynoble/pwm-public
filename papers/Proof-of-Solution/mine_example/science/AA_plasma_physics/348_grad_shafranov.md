# Grad Shafranov — Complete Four-Layer Walkthrough
...
## The Four-Layer Pipeline for Grad Shafranov
...
```
┌────────────────────────────────────────────────────────────────────────────┐
│  GRAD SHAFRANOV PRINCIPLE  P = (E, G, W, C)                      │
│  Principle #348 in the PWM registry                                        │
│  sha256: <grad_shafranov_principle_hash>  (immutable once committed)          │
├────────┬───────────────────────────────────────────────────────────────────┤
│   E    │ FORWARD MODEL                                                    │
│        │                                                                  │
...
```
Grad Shafranov Principle (sha256:<principle_hash>)
    │
...
```yaml
# grad_shafranov/fluocells_mismatch_only.yaml
# Layer 2 output — references the Grad Shafranov Principle

principle_ref: sha256:<grad_shafranov_principle_hash>   # ← links to Layer 1
...
```yaml
# grad_shafranov/fluocells_oracle_assisted.yaml

principle_ref: sha256:<grad_shafranov_principle_hash>
...
Each spec.md is validated against the Grad Shafranov Principle:
...
```
benchmark_grad_shafranov_mismatch_only_t1_nominal/   ← I-benchmark T1
...
```yaml
# benchmark_grad_shafranov_mismatch_only_t1_nominal/manifest.yaml

benchmark_id:    "grad_shafranov_mismatch_only_t1_nominal_v1"
type:            "I-benchmark"
spec_ref:        "sha256:<grad_shafranov_spec1_hash>"       # mismatch-only spec
principle_ref:   "sha256:<grad_shafranov_principle_hash>"
...
dataset_hash:    "sha256:<grad_shafranov_dataset_hash>"
generator_hash:  "sha256:<grad_shafranov_gen_hash>"
...
```bash
pwm-node verify grad_shafranov/fluocells_t1_nominal.yaml
```
...
```bash
pwm-node mine grad_shafranov/fluocells_t1_nominal.yaml
```
...
{
  "cert_hash": "sha256:...",
  "h_s": "sha256:<grad_shafranov_spec1_hash>",
  "h_b": "sha256:<grad_shafranov_bench1_hash>",
  "h_p": "sha256:<grad_shafranov_principle_hash>",
...
```
cert references:
  h_p → Principle  sha256:<grad_shafranov_principle_hash>   (Layer 1, immutable)
  h_s → spec.md    sha256:<grad_shafranov_spec1_hash>       (Layer 2, immutable)
  h_b → Benchmark  sha256:<grad_shafranov_bench1_hash>      (Layer 3, immutable)
  h_x → Solution   sha256:<grad_shafranov_solution_hash>    (Layer 4, this submission)
```
...
```
Layer 1 ──→ Principle sha256:<grad_shafranov_principle_hash>     FIXED
               │
Layer 2 ──→ spec.md sha256:<grad_shafranov_spec1_hash>           FIXED
               │   contains: principle_ref: sha256:<grad_shafranov_principle_hash>
               │
Layer 3 ──→ Benchmark sha256:<grad_shafranov_bench1_hash>        FIXED
               │   contains: spec_ref: sha256:<grad_shafranov_spec1_hash>
               │             principle_ref: sha256:<grad_shafranov_principle_hash>
               │
Layer 4 ──→ Certificate sha256:<grad_shafranov_cert_hash>        SUBMITTED
                   contains: h_s: sha256:<grad_shafranov_spec1_hash>
                             h_b: sha256:<grad_shafranov_bench1_hash>
                             h_p: sha256:<grad_shafranov_principle_hash>
                             h_x: sha256:<grad_shafranov_solution_hash>
...
```bash
pwm-node mine grad_shafranov/fluocells_mismatch_only_t1_nominal.yaml
```
...
## Complete Reward Summary (All Four Layers for Grad Shafranov)
...
pwm-node verify grad_shafranov/mismatch_only_t1_nominal.yaml

# 3. Mine the center I-benchmark (nominal, ρ=1)
pwm-node mine grad_shafranov/mismatch_only_t1_nominal.yaml
...
pwm-node mine grad_shafranov/mismatch_only_t3_moderate.yaml

# 7. Mine blind calibration tier (ρ=10, highest I-benchmark)
pwm-node mine grad_shafranov/mismatch_only_t4_blind.yaml

# 8. Mine oracle-assisted center I-benchmark (ρ=1, true_phi provided)
pwm-node mine grad_shafranov/oracle_t1_h128_pix100.yaml
...
|-------|------------------|
| Grad Shafranov Principle (#1) | `principle.md`, Section A (Microscopy) |
