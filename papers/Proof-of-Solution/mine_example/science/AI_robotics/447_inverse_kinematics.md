# Inverse Kinematics — Complete Four-Layer Walkthrough
...
## The Four-Layer Pipeline for Inverse Kinematics
...
```
┌────────────────────────────────────────────────────────────────────────────┐
│  INVERSE KINEMATICS PRINCIPLE  P = (E, G, W, C)                      │
│  Principle #447 in the PWM registry                                        │
│  sha256: <inverse_kinematics_principle_hash>  (immutable once committed)          │
├────────┬───────────────────────────────────────────────────────────────────┤
│   E    │ FORWARD MODEL                                                    │
│        │                                                                  │
...
```
Inverse Kinematics Principle (sha256:<principle_hash>)
    │
...
```yaml
# inverse_kinematics/fluocells_mismatch_only.yaml
# Layer 2 output — references the Inverse Kinematics Principle

principle_ref: sha256:<inverse_kinematics_principle_hash>   # ← links to Layer 1
...
```yaml
# inverse_kinematics/fluocells_oracle_assisted.yaml

principle_ref: sha256:<inverse_kinematics_principle_hash>
...
Each spec.md is validated against the Inverse Kinematics Principle:
...
```
benchmark_inverse_kinematics_mismatch_only_t1_nominal/   ← I-benchmark T1
...
```yaml
# benchmark_inverse_kinematics_mismatch_only_t1_nominal/manifest.yaml

benchmark_id:    "inverse_kinematics_mismatch_only_t1_nominal_v1"
type:            "I-benchmark"
spec_ref:        "sha256:<inverse_kinematics_spec1_hash>"       # mismatch-only spec
principle_ref:   "sha256:<inverse_kinematics_principle_hash>"
...
dataset_hash:    "sha256:<inverse_kinematics_dataset_hash>"
generator_hash:  "sha256:<inverse_kinematics_gen_hash>"
...
```bash
pwm-node verify inverse_kinematics/fluocells_t1_nominal.yaml
```
...
```bash
pwm-node mine inverse_kinematics/fluocells_t1_nominal.yaml
```
...
{
  "cert_hash": "sha256:...",
  "h_s": "sha256:<inverse_kinematics_spec1_hash>",
  "h_b": "sha256:<inverse_kinematics_bench1_hash>",
  "h_p": "sha256:<inverse_kinematics_principle_hash>",
...
```
cert references:
  h_p → Principle  sha256:<inverse_kinematics_principle_hash>   (Layer 1, immutable)
  h_s → spec.md    sha256:<inverse_kinematics_spec1_hash>       (Layer 2, immutable)
  h_b → Benchmark  sha256:<inverse_kinematics_bench1_hash>      (Layer 3, immutable)
  h_x → Solution   sha256:<inverse_kinematics_solution_hash>    (Layer 4, this submission)
```
...
```
Layer 1 ──→ Principle sha256:<inverse_kinematics_principle_hash>     FIXED
               │
Layer 2 ──→ spec.md sha256:<inverse_kinematics_spec1_hash>           FIXED
               │   contains: principle_ref: sha256:<inverse_kinematics_principle_hash>
               │
Layer 3 ──→ Benchmark sha256:<inverse_kinematics_bench1_hash>        FIXED
               │   contains: spec_ref: sha256:<inverse_kinematics_spec1_hash>
               │             principle_ref: sha256:<inverse_kinematics_principle_hash>
               │
Layer 4 ──→ Certificate sha256:<inverse_kinematics_cert_hash>        SUBMITTED
                   contains: h_s: sha256:<inverse_kinematics_spec1_hash>
                             h_b: sha256:<inverse_kinematics_bench1_hash>
                             h_p: sha256:<inverse_kinematics_principle_hash>
                             h_x: sha256:<inverse_kinematics_solution_hash>
...
```bash
pwm-node mine inverse_kinematics/fluocells_mismatch_only_t1_nominal.yaml
```
...
## Complete Reward Summary (All Four Layers for Inverse Kinematics)
...
pwm-node verify inverse_kinematics/mismatch_only_t1_nominal.yaml

# 3. Mine the center I-benchmark (nominal, ρ=1)
pwm-node mine inverse_kinematics/mismatch_only_t1_nominal.yaml
...
pwm-node mine inverse_kinematics/mismatch_only_t3_moderate.yaml

# 7. Mine blind calibration tier (ρ=10, highest I-benchmark)
pwm-node mine inverse_kinematics/mismatch_only_t4_blind.yaml

# 8. Mine oracle-assisted center I-benchmark (ρ=1, true_phi provided)
pwm-node mine inverse_kinematics/oracle_t1_h128_pix100.yaml
...
|-------|------------------|
| Inverse Kinematics Principle (#1) | `principle.md`, Section A (Microscopy) |
