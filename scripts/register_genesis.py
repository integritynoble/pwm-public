...
# Canonical-JSON hashing is shared with pwm_node.commands.mine via
# pwm_node.canonical so the two consumers can't drift. Adding a new
# UI-only field is a one-line edit in canonical.py — both registration
# and mining pick it up automatically.
#
# Resolve the agent-cli package path so this script works whether or
# not the venv has pwm-node installed (founders running `python3
# scripts/register_genesis.py` outside an active venv).
_AGENT_CLI = (
    Path(__file__).resolve().parent.parent
    / "pwm-team" / "infrastructure" / "agent-cli"
)
if _AGENT_CLI.is_dir() and str(_AGENT_CLI) not in sys.path:
    sys.path.insert(0, str(_AGENT_CLI))
from pwm_node.canonical import (  # noqa: E402  (sys.path setup above is intentional)
    UI_ONLY_FIELDS,
