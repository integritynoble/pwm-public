"""CACTI reference-solver quality test.

Compares the stored solution.npz (reference-solver output) against
ground_truth.npz for every sample under
`pwm-team/pwm_product/demos/cacti/sample_*` and reports per-sample
PSNR (dB) plus the average.

Per `pwm-team/coordination/MAINNET_DEPLOY_PLAN.md` Stream 1 done
criterion (a): average PSNR over SCI-6 must be >= 24 dB.

Usage:
    cd pwm-team/pwm_product && python -m pytest tests/test_cacti_quality.py -v
    # or just print PSNRs without assertion:
    python pwm-team/pwm_product/tests/test_cacti_quality.py
"""
...
PSNR_TARGET_DB = 24.0
DEMO_DIR = Path(__file__).resolve().parents[1] / "demos" / "cacti"
GT_KEYS = ("x", "gt", "ground_truth", "video", "frames")
SOL_KEYS = ("video", "x", "solution", "x_hat", "frames", "y")
...
def test_cacti_average_psnr_meets_target():
    """Average PSNR over all CACTI samples must clear the deploy gate."""
    summary = _summary()
    rows = summary["rows"]
    assert rows, f"no CACTI samples found at {DEMO_DIR}"
    print()
    print(f"CACTI per-sample PSNR (target average >= {PSNR_TARGET_DB} dB)")
    for name, psnr, meta in rows:
        marker = "✓" if psnr >= PSNR_TARGET_DB else "✗"
        scene = meta.get("scene_id", "")
        print(f"  {marker} {name} [{scene}]: {psnr:6.2f} dB")
    avg = summary["avg_psnr_db"]
    print(f"  avg = {avg:6.2f} dB ({summary['count']} samples)")
    assert avg >= PSNR_TARGET_DB, (
        f"CACTI avg PSNR {avg:.2f} dB < target {PSNR_TARGET_DB} dB "
...
if __name__ == "__main__":
    summary = _summary()
    print(f"CACTI per-sample PSNR (target average >= {PSNR_TARGET_DB} dB)")
    for name, psnr, meta in summary["rows"]:
