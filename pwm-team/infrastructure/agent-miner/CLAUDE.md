...
- `../../coordination/agent-coord/interfaces/contracts_abi/` — PWMCertificate event stream
- `pwm_overview1.md` §Compute Provider, §Commit-then-Reveal, §Spot-Check — your specification
...
Implements the anti-front-running commit-then-reveal protocol (60-second window):
```
...
## Spot-check verification (from protocol)
- 5% of all completed jobs are re-run by protocol validators
- Match: CP reputation +1; no action
- Mismatch: CP slashed 50%; 50% to validator; job reward clawed back
- Three mismatches in 30 days: 100% slash + address banned
