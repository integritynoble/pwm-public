...
```
    ...
    │        │                                                                     │
    │        │ y_k = ⟨φ_k, x⟩ + n_k             (per-measurement form)            │
    │        │ y   = Φ x + n                    (stacked matrix form)             │
    │        │      Φ ∈ ℝ^{m×n}  Walsh-Hadamard rows in {-1,+1}                  │
    │        │      x ∈ ℝⁿ       (vectorised image, n = H·W)                     │
    │        │      y ∈ ℝᵐ       (m bucket-detector readings)                    │
    │        │      n ∼ 𝒩(0, σ²) (additive Gaussian)                              │
    │        │                                                                     │
    │        │ Physical chain (matches G's DAG below):                            │
    │        │   Scene x ──→ Modulate by Hadamard pattern φ_k ──→                │
    │        │   Integrate on bucket detector ──→ Scalar reading y_k             │
    │        │                                                                     │
    │        │ In the canonical 12-primitive basis (primitives.md):               │
    │        │   y_k = ∫.spatial ∘ L.diag.binary( G.structured.random(k), x )    │
    │        │         + n_k                                                       │
    │        │ Strict signature (primitives.md line 567):                         │
    │        │   #26 SPC = [L.diag.binary] → [∫.spatial]                         │
    │        │                                                                     │
    │        │ Inverse problem: recover x ∈ ℝⁿ from y ∈ ℝᵐ, m << n.              │
    │        │ Compression ratio: n/m (e.g. 4:1 at 25% sampling).                │
    ├────────┼────────────────────────────────────────────────────────────────────┤
